...
def infer_pdf( src ): 
    ...
    docs=text_splitter.split_documents(src)
    embeddings=HuggingFaceEmbeddings(model_name='sentence-transformers/all-MiniLM-L6-v2')    
    docsearch= pcone.from_texts([t.page_content for t in docs], embeddings, index_name=index_name)

    query="Generate 3 MCQ questions with answers based on the content given in PDF"
...
async def generate_mcq( pdf_text: str ):
    try:
        start = time.time() 
        infer_pdf(pdf_text)
